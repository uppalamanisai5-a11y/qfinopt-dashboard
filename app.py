...
with tab3:
    ...
    st.caption(
        "Historical NAV with an illustrative scenario estimate. "
    ...
    def resolve_mfapi_code(fund_name):
        try:
            ...
            def score(item):
                available_words = set(
                    item.get("schemeName", "").lower().split()
                )
                return len(wanted_words.intersection(available_words))

            best_match = max(matches, key=score)
            return str(best_match.get("schemeCode")) if best_match.get("schemeCode") else None
        except Exception:
            return None
    ...
    def get_live_nav_history(scheme_code):
        ...
        try:
            ...
            live_data = pd.DataFrame(history)
            live_data["Date"] = pd.to_datetime(
                live_data["date"], dayfirst=True, errors="coerce"
            )
            live_data["NAV"] = pd.to_numeric(
                live_data["nav"], errors="coerce"
            )
            ...
            return live_data if len(live_data) > 5 else None
        except Exception:
            return None

    mfapi_code = resolve_mfapi_code(selected_fund)
    live_history = get_live_nav_history(mfapi_code)
    ...
    forecast_days = st.slider(
        "Illustrative forecast period (trading days)",
        min_value=5, max_value=90, value=30, step=5,
        key="tab3_forecast_days"
    )

    if live_history is not None:
        full_data = live_history.copy()
        source_label = "📡 Live NAV history from AMFI / mfapi.in"
        source_short = "Live"
    else:
        full_data = fund_df[["Date", "NAV_Value"]].copy()
        full_data = full_data.rename(columns={"NAV_Value": "NAV"})
        source_label = "📁 Historical CSV — live NAV unavailable"
        source_short = "Historical"
    ...
    latest_nav = float(full_data["NAV"].iloc[-1])
    latest_date = pd.Timestamp(full_data["Date"].iloc[-1])
    data_age_days = (
        pd.Timestamp.now().normalize() - latest_date.normalize()
    ).days

    max_days = period_map[period]
    chart_data = (
        full_data.tail(max_days).copy()
        if max_days < len(full_data)
        else full_data.copy()
    )

    base_nav = float(chart_data["NAV"].iloc[0])
    percent_change = (chart_data["NAV"] / base_nav - 1) * 100
    daily_returns = full_data["NAV"].pct_change().dropna() * 100

    # Bootstrap scenario estimate from actual historical daily returns.
    historical_returns = daily_returns.to_numpy() / 100
    rng = np.random.default_rng(42)
    ...
    if len(historical_returns) >= 30:
        ...
        simulated_paths = latest_nav * np.cumprod(
            1 + sampled_returns,
            axis=1
        )
        ...
        predicted_nav = float(np.median(final_values))
        forecast_low = float(np.percentile(final_values, 10))
        forecast_high = float(np.percentile(final_values, 90))
        predicted_change = ((predicted_nav / latest_nav) - 1) * 100
    ...
    if predicted_nav is not None:
        m3.metric(
            f"Estimated NAV (+{forecast_days}d)",
            f"₹{predicted_nav:.2f}",
            f"{predicted_change:+.2f}%"
        )
        m4.metric(
            "Likely range (10–90%)",
            f"₹{forecast_low:.2f} – ₹{forecast_high:.2f}"
        )
    else:
        m3.metric("Estimated NAV", "Not enough history")
        m4.metric("Likely range", "Not enough history")

    m5.metric("NAV date", latest_date.strftime("%d %b %Y"))

    if data_age_days > 7:
        st.warning(
            f"⚠️ Latest available NAV: {latest_date.strftime('%d %b %Y')} "
    ...
    if predicted_nav is not None:
        st.info(
            f"Illustrative estimate for "
            f"{future_dates[-1].strftime('%d %b %Y')}: "
            f"₹{predicted_nav:.2f}. "
            f"Historical-simulation range: ₹{forecast_low:.2f} to "
            f"₹{forecast_high:.2f}. "
            f"This is not a guaranteed return."
        )
    ...
    with col1:
        ...
        if predicted_nav is not None:
            forecast_percent = (
                (np.asarray(median_path) / base_nav - 1) * 100
            )

            figure.add_trace(go.Scatter(
                x=[latest_date] + future_dates,
                y=[percent_change.iloc[-1]] + list(forecast_percent),
                mode="lines",
                name="Illustrative estimate",
                line=dict(color="orange", width=2.5, dash="dash"),
                hovertemplate=(
                    "%{x|%d %b %Y}<br>"
                    "<b>%{y:.2f}%</b><extra>Estimate</extra>"
                )
            ))
