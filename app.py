...
# ── TAB 3: Fund Analysis — Live % Change + Forecast + Trend ──
with tab3:
    st.subheader(f"📈 {selected_fund[:55]}")

    scheme_code = str(fund_df["Scheme_Code"].iloc[-1]).split(".")[0]
    ...
    def get_live_nav_history(code):
        ...
        try:
            ...
            return hdf if len(hdf) > 5 else None
        except Exception:
            return None
    ...
    with col1:
        ...
        ax3.plot(dates, values, color="steelblue", linewidth=1.8, label="Actual (% change)")

        if show_forecast3 and len(values) > 0:
            last_val, last_date = values[-1], dates[-1]
            fut_dates = pd.bdate_range(
                last_date + pd.Timedelta(days=1), periods=forecast_days3)
            fut_vals  = [last_val + mu_trend*(i+1) for i in range(forecast_days3)]
            ax3.plot([last_date]+list(fut_dates), [last_val]+fut_vals,
                     color="orange", linewidth=1.8, linestyle="--", label="Predicted")

        ax3.axhline(0, color="black", linewidth=0.8, alpha=0.4)
        ax3.set_title(f"% Change — {period}  ({trend_icon} {trend_label})")
        ax3.set_xlabel("Date")
